...
def edit_screen(information, organized_combo, original_menu):
    while True:
        ...
        if user_choice == "Confirm":
            # Adds new information or combo to the menu
...
def menu_totals(original_menu):
    ...
    formatted_menu = ""
    # Runs through each combo on the menu
    for combo, items in original_menu.items():
        # Adds the total price of each combo together
        total_price = sum(items.values())
        # Adds the total to each combo on the dictionary formatter then adds
...
def delete_combo(full_menu):
    # Gets the dictionary searched and the combo name(key)
    delete_info, delete_name = search_menu(full_menu, "Delete combo")
    # Formats the dictionary to use in the easygui statements
    present_combo = dictionary_formatter(delete_info)
    # Checks if the combo name is on the menu
    if delete_name in full_menu:
        ...
        if choice == "Yes":
            ...
            confirm = easygui.buttonbox(f"Confirm you want to delete "
                                        ...
                                        ["Delete", "Cancel"])
            if confirm == "Delete":
                # Deletes combo from menu if selected
